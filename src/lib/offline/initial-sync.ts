...
async function fetchAllData(): Promise<void> {
  const [actions, inbox, projects, dailyNotes, routine, referenceDocs, disciplines, disciplineLogs, contextLists, dailyBlocksRes] = await Promise.all([
  ...
  await offlineDb.transaction('rw', tables, async () => {
    ...
    const syncTables = [
      ...
      'disciplines', 'discipline_logs', 'context_lists', 'daily_blocks',
    ];
